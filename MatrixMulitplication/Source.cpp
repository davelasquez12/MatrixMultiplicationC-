...
int main()
{
	...
	for (int r = 0; r < matrix1_numofRows; r++)
	{
		...
		for (int c = 0; c < matrix2_numofColumns; c++)		//***this loop is what was needed****
			result[r][c] = 0;
	}
		
	//matrixAfterMultiply(matrix1, matrix2, result, matrix1_numofRows, matrix1_numofColumns, matrix2_numofColumns);
	matrixAfterMultiplyWhileLoops(matrix1, matrix2, result, matrix1_numofRows, matrix1_numofColumns, matrix2_numofColumns);
