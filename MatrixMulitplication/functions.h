...
void matrixAfterMultiplyWhileLoops(int ** m1, int ** m2, int ** newMatrix, int row_matrix1, int column_matrix1, int column_matrix2)
{
	int rw = 0;
	while (rw < row_matrix1)
	{
		int cl = 0;
		while (cl < column_matrix2)
		{
			int s = 0;
			while (s < column_matrix1)
			{
				newMatrix[rw][cl] += m1[rw][s] * m2[s][cl];
				s++;
			}
	...
	cout << "After multiplication using while loops, your new matrix contains: " << endl;
	displayMatrix(newMatrix, row_matrix1, column_matrix2);
}
